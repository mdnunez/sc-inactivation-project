...
returnListOfModels = function()
{
  modelList = c(
    ...
    "cDDM",                    # -7, DDM with collapsing boundaries
    "cDDMSv",                  # -8
    "cDDMSt",                  # -9
    "cDDMSvSz",                # -10
    "cDDMSvSt",                # -11
    ...
    "dDDM",                    # -13, DDM with Ditterich's urgency 
    "dDDMSt",                  # -14
    "dDDMSv",                  # -15
    "dDDMSvSt",                # -16
    "dDDMSvSzSt",              # -17
    
    "cfkDDM",                  # -18, DDM with collapsing bounds with scale f fixed
    "cfkDDMSt",                # -19
    "cfkDDMSvSt",              # -20
    "cfkDDMSvSzSt",            # -21
    ...
    "bUGM",                    #  5, Cisek UGM with intercept
    "bUGMSt",                  #  6
    "bUGMSv",                  #  7       
    "bUGMSvSt",                #  8       
    "bUGMSvSb",                #  9       
...
paramsandlims=function(model, nds, fakePars=FALSE, nstart=1,fullrangefit,fixed_params_df)
{
  ...
  fakeParsUGM = c(seq(1.5, 15,  length.out=NdriftRates) + 0.5*rnorm(nds), 0.6, 15000, 0.3, 4, 0.1, 1000, 600, 200,2, 5, 0.3, 10);
  ...
  switch(model, 
         DDM={ # 7 coherences, an upper bound, and a lower bound, symmetric bounds
           parnames=c(paste("v",(nstart):(nds),sep=""),"z", "aU","Ter")
         ...
         cDDM={
           parnames=c(paste("v",(nstart):(nds),sep=""),"z", "aU","Ter","lambda","aprime","k")
         ...
         bUGM={
           parnames=c(paste("v",(nstart):(nds),sep=""), "z","aU","Ter","intercept")
         ...
         mbUGM={
           parnames=c(paste("v",(nstart):(nds),sep=""), "z","aU","Ter","intercept",'usign_var')
           print("Urgency Gating with an intercept and slope")           
         },
         mUGM={
           parnames=c(paste("v",(nstart):(nds),sep=""),"z","aU","Ter",'usign_var')
           print("Urgency Gating with an intercept and slope")           
         }
  )  
  
  
  parUppersDDM = c(rep(upper_v_ddm,NdriftRates), upper_z, upper_aU_ddm, upper_Ter, upper_eta, upper_st0,
                   upper_zmin, upper_zmax, 50, 50, 3, 10,upper_aprime,20);
  
  parUppersUGM = c(rep(upper_v_urgency,NdriftRates), upper_z, upper_aU_urgency, upper_Ter, upper_eta_urgency, upper_st0,
  ...
  if (fitUGM > 0) {
    lowers[1:NdriftRates] = -upper_v_urgency
  } else {
  ...
  if (fullrangefit != 1) {
    ...
    } else {           #fitting DDM with non-full range fit 
      uppers[1] = 0.2   #-24
      uppers[2] = 0.4   #-17
      uppers[3] = 0.6 #-10
      uppers[4] = 0.6 #-5
  ...
  # Set lower limit for Ter to be 0.05, otherwise you can get very low Ter values
  ...
  if  (sum(fixed_params_df['fixed_params']) != 0) {    #If there are some fixed parameters
    ...
    for (n in 1:sum(fixed_params_df['fixed_params'])) {
      fixed_params_names_list = c(fixed_params_names_list,as.character(fixed_params_df[['names']][[indices_of_1[n]]]))
    }
    for (n in 1:sum(fixed_params_df['fixed_params'])) {
      takeout_index = which(parnames==fixed_params_names_list[n])
      parnames = parnames[-(takeout_index)]
      uppers = uppers[-(takeout_index)]
...
# lambda, aprime, k - parameters for the collapsing bounds model
{
  ...
  switch(modelName,
         ...
         mbUGM={   #essentially just the bUGM but with variable slope too
           out=.C("bUGM",z=z,v=v,aU=aU,aL=aL,timecons=timecons,
                  usign=usign_var,intercept=intercept, s=stoch.s,dt=dt,
                  response=resps,rt=rts,n=nmc,maxTimeStep=maxTimeStep,
                  rangeLow =as.integer(0), rangeHigh = as.integer(nLUT-1), randomTable = as.double(LUT));
           rts=(out$rt/1000) + Ter;
         },
         mUGM={  #essentially just the UGM but with variable slope
           out=.C("UGM",z=z,v=v,aU=aU,aL=aL, timecons=timecons, usign=usign_var, s=stoch.s,dt=dt,
                  response=resps,rt=rts,n=nmc,maxTimeStep=maxTimeStep,
                  rangeLow =as.integer(0), rangeHigh = as.integer(nLUT-1), randomTable = as.double(LUT));
           rts=(out$rt/1000) + Ter;
         }
...
makeparamlist=function(params,fitUGM,ncohs, zeroCoh=FALSE,fixed_params_df) 
{
  ...
  if  (sum(fixed_params_df['fixed_params']) != 0) {    #If there are some fixed parameters
    ...
    if (sum(fixed_params_names_list == 'v1') == 1) {
      v[1] = fixed_params_df[['fixed_values']][[which(fixed_params_df['names'] == 'v1')]]
    }
    if (sum(fixed_params_names_list == 'v2') == 1) {
      v[2] = fixed_params_df[['fixed_values']][[which(fixed_params_df['names'] == 'v2')]]
    }
    if (sum(fixed_params_names_list == 'v3') == 1) {
      v[3] = fixed_params_df[['fixed_values']][[which(fixed_params_df['names'] == 'v3')]]
    }
    if (sum(fixed_params_names_list == 'v4') == 1) {
      v[4] = fixed_params_df[['fixed_values']][[which(fixed_params_df['names'] == 'v4')]]
    }
    if (sum(fixed_params_names_list == 'v5') == 1) {
      v[5] = fixed_params_df[['fixed_values']][[which(fixed_params_df['names'] == 'v5')]]
    }
    if (sum(fixed_params_names_list == 'v6') == 1) {
      v[6] = fixed_params_df[['fixed_values']][[which(fixed_params_df['names'] == 'v6')]]
    }
    if (sum(fixed_params_names_list == 'v7') == 1) {
      v[7] = fixed_params_df[['fixed_values']][[which(fixed_params_df['names'] == 'v7')]]
    }
    if (sum(fixed_params_names_list == 'v8') == 1) {
      v[8] = fixed_params_df[['fixed_values']][[which(fixed_params_df['names'] == 'v8')]]
    }
    if (sum(fixed_params_names_list == 'v9') == 1) {
      v[9] = fixed_params_df[['fixed_values']][[which(fixed_params_df['names'] == 'v9')]]
    }
    if (sum(fixed_params_names_list == 'v10') == 1) {
      v[10] = fixed_params_df[['fixed_values']][[which(fixed_params_df['names'] == 'v10')]]
    }
    if (sum(fixed_params_names_list == 'v11') == 1) {
      v[11] = fixed_params_df[['fixed_values']][[which(fixed_params_df['names'] == 'v11')]]
    }
    if (sum(fixed_params_names_list == 'aU') == 1) {
      
      aU = fixed_params_df[['fixed_values']][[which(fixed_params_df['names'] == 'aU')]] 
      if (fitUGM < 0) {
        aL = 0
        z = params["z"]*aU
      } else{
    ...
    if (sum(fixed_params_names_list == 'z') == 1) {
      if(fitUGM < 0) {
        z = fixed_params_df[['fixed_values']][[which(fixed_params_df['names'] == 'z')]]*aU
      } else {
        z=fixed_params_df[['fixed_values']][[which(fixed_params_df['names'] == 'z')]]*(2*aU)+aL
      }
    } 
    
    if (sum(fixed_params_names_list == 'Ter') == 1) {
      Ter = fixed_params_df[['fixed_values']][[which(fixed_params_df['names'] == 'Ter')]]
    } 
    if (sum(fixed_params_names_list == 'lambda') == 1) {
      lambda = fixed_params_df[['fixed_values']][[which(fixed_params_df['names'] == 'lambda')]]
    } 
    if (sum(fixed_params_names_list == 'aprime') == 1) {
      aprime = fixed_params_df[['fixed_values']][[which(fixed_params_df['names'] == 'aprime')]]
    } 
    if (sum(fixed_params_names_list == 'k') == 1) {
      k = fixed_params_df[['fixed_values']][[which(fixed_params_df['names'] == 'k')]]
    } 
    if (sum(fixed_params_names_list == 'intercept') == 1) {
      intercept = fixed_params_df[['fixed_values']][[which(fixed_params_df['names'] == 'intercept')]]
    } 
