function [first_saccade_time_spec, peak_vel_spec, Trial_1] = Saccade_Detector_STO_spec(Trial_nsx, Trial_1, saccadeTimeFrame_0_spec,saccadeTimeFrame_1_spec, velocityThreshold_spec, accelerationThreshold_spec, w, allh, allv, FirstPassThresholds_spec)
...
n = w         %This is excluding the last trial
    ...
    col1 = find(codes == 3000);     %structure index for the 5000 codes; GPOn
    
    
    all_vertical_positions(1,1:matrix_columns_size) = allv(1,Trial_1(n).timeindexes(col1) + saccadeTimeFrame_0_spec*30: Trial_1(n).timeindexes(col1)+saccadeTimeFrame_1_spec*30);
    all_horizontal_positions(1,1:matrix_columns_size) = allh(1,Trial_1(n).timeindexes(col1) + saccadeTimeFrame_0_spec*30: Trial_1(n).timeindexes(col1)+saccadeTimeFrame_1_spec*30);
    alltimes_index(1,1:matrix_columns_size) = Trial_1(n).timeindexes(col1) + saccadeTimeFrame_0_spec*30: Trial_1(n).timeindexes(col1)+ saccadeTimeFrame_1_spec*30;
...
%     Trial_1(w).all_vertical_positions_sized = all_vertical_positions_sized(w,:);
%     Trial_1(w).all_horizontal_positions_sized = all_horizontal_positions_sized(w,:);
%     Trial_1(w).alltimes_index_sized = alltimes_index_sized(w,:);
%%
...
stop = 1
    ...
    if isnan(saccade_index)
        ...
        vel_accel_around_sac = [velocity_diagonal(saccade_index - 30: saccade_index)';accelerations(saccade_index - 30: saccade_index)'];
