# mky3_PrePostRecHalf.py - Loads ALL behavioral data from Pre- Post- and Recovery-from Muscimol recordings 
#                            Model with all variables free across sessions but only modeling rightwardRT
#
...
# ====         ================                       ======================
# 01/29/20      Michael Nunez          Converted from mky3_PrePostRecmodel1.py
# 01/30/20      Michael Nunez                Increase lower bound of initial values of problapse
# 02/02/20      Michael Nunez             Fix size of monkey and inject arrays
# 02/04/20      Michael Nunez          Remove injection conditions without any samples to avoid sampler getting stuck
#                                    Simplify model to remove dimensions from standard deviation parameters 
# 02/05/20      Michael Nunez            Save out inject_labels in samples dictionary
# 02/06/20      Michael Nunez          Run model with injection condition collapsed across side
# 02/11/20      Michael Nunez                Execute proper mky3_modelresults.py definitions
# 02/19/20      Michael Nunez                     Monitor Deviance Information Criterion (dic)\
# 02/20/20      Michael Nunez                Set constant value to 0, change initial values to avoid "Invalid Parent Values"
# 02/21/20      Michael Nunez         Increase constant value to 20, remove dic (not valid for JAGS models with ones trick)
# 04/10/20      Michael Nunez        Run training data from fixedexpSCInjectGPRTdata by default
...
model {
    ...
    for (e in 1:ninject) {
        for (m in 1:nmonkeys) {
            ...
            deltarighthier[e,m] ~ dnorm(0, pow(2, -2))
        }
    }
