# mky3_PrePostRecLinearDrift.py - Loads ALL behavioral data from Pre- Post- and Recovery-from Muscimol recordings 
#        Model with all variables free across sessions, only modeling rightwardRT, with a linear relationship across drift rates
#
...
# ====         ================                       ======================
# 03/04/21      Michael Nunez          Converted from mky3_PrePostRec2Accum.py
...
coherence = np.array([0., -0.03, -0.05, -0.10, -0.17, -0.24, -0.36, 0.03, 0.05, 0.10, 0.17, 0.24, 0.36])
...
trackvars = ['proportionality', 'driftbias',  
...
for c in range(0, nchains):
    chaininit = {
        'proportionality': np.random.uniform(0., 5., size=(ninject,nmonkeys)),
        'driftbias': np.random.uniform(-4., 4., size=(ninject,nmonkeys)),
...
indata = dict(N=N, rightwardRT=rightwardRT, condition=condition, session=session, monkey=monkey, inject=inject, coherence=coherence,
