# mky3_DayHalfSPBound.py - Loads ALL behavioral data from Pre- Post- and Recovery-from Muscimol recordings 
#                            Model with all variables free across days, only modeling rightwardRT, with start point and boundary free across sessions
#
...
# ====         ================                       ======================
# 06/05/20      Michael Nunez          Converted from mky3_DayHalfSPFree.py
...
model {
    ...
    for (i in 1:N) {
        ...
        ld_comp2[i, 1] <- dlogwiener(rightwardRT[i], alpha[session[i]], ter[day[i]], beta[session[i]], deltaright[day[i],condition[i]])
