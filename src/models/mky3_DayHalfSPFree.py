# mky3_DayHalfDriftFree.py - Loads ALL behavioral data from Pre- Post- and Recovery-from Muscimol recordings 
#                            Model with all variables free across days, only modeling rightwardRT,  and start point free across sessions
#
...
# ====         ================                       ======================
# 02/20/20      Michael Nunez          Converted from mky3_DayHalfDriftFree.py
# 02/21/20      Michael Nunez         Increase constant value to 20, remove dic (not valid for JAGS models with ones trick)
# 04/13/20      Michael Nunez                Adaptive home directory and change default data file
# 04/14/20      Michael Nunez                 Use of training data
...
minrt = np.empty((ndays))
for d in np.unique(day):
    monkeyday[d-1] = monkeylarge[day == d][0]
    minrt[d-1] = np.min(np.abs(rightwardRT[day==d]))
...
model {
    ...
    for (d in 1:ndays) {
        #Non-decision time
        ter[d] ~ dnorm(terhier[monkeyday[d]], pow(tersd,-2))T(0, 1)
    ...
    for (i in 1:N) {
        ...
        ld_comp2[i, 1] <- dlogwiener(rightwardRT[i], alpha[day[i]], ter[day[i]], beta[session[i]], deltaright[day[i],condition[i]])
...
for c in range(0, nchains):
    chaininit = {
        ...
        'terhier': np.random.uniform(0., .5, size=(nmonkeys)),
        ...
        'ter': np.random.uniform(0., .5, size=(ndays)),
    ...
    for d in range(0, ndays):
            chaininit['ter'][d] = np.random.uniform(0., minrt[d]/2)
