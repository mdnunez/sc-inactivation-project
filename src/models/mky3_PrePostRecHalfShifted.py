# mky3_PrePostRecHalfShifted.py - Loads ALL behavioral data from Pre- Post- and Recovery-from Muscimol recordings 
...
model {
    ...
    for (e in 1:ninject) {
        for (m in 1:nmonkeys) {
            ...
            terhier[e,m] ~ dnorm(1, pow(.25,-2))

            #Hierarchical Speed-accuracy trade-off
            alphahier[e,m] ~ dnorm(2, pow(.5,-2))

            #Hierarchical Rightward start point bias
            betahier[e,m] ~ dnorm(.25, pow(.25,-2))

            #Hierarchical lapse trial probability
            problapsehier[e,m] ~ dnorm(.4, pow(.15,-2))

            #Hierarchical Rightward/Leftward drift rate
            deltarighthier[e,m] ~ dnorm(1, pow(2, -2))
        }
    }
