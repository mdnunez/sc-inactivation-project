function [Trial_3, remove_3] = SacDetectManualCheck_GP(Trial, Trial_3,DT, RT, saccadeTimeFrame_0,saccadeTimeFrame_1, velocityThreshold,accelerationThreshold,allh, allv)
...
nPlotsBunch = 50; %how many trials to plot at one time 
t = 1:nPlotsBunch:numel(Trial_3);
input_plot = 'y';
...
while input_plot1 ~= 'y' && input_plot1 ~= 'n'
    ...
    if input_plot1 == 'y'
        for i = 1:ceil(numel(Trial_3)/nPlotsBunch)
            if input_plot == 'y'
                if i == ceil(numel(Trial_3)/nPlotsBunch)
                    w = t(i):numel(Trial_3);
                else
                    w = t(i):t(i+1)-1;                %vector of the trial numbers needed to be plotted, 50 at a time
                end
                analog_signal_plotter_GP(Trial,Trial_3,saccadeTimeFrame_0, w, DT, RT);
...
if input_doyou == 'y'
    ...
    for c = 1:numel(lookat)       %for each trial you want to look at
        ...
        while tryagain == 1
            w = lookat(c);
            analog_signal_plotter_GP(Trial,Trial_3,saccadeTimeFrame_0, w, DT, RT);    %graph the specific trial out
            discard_input = 'z';
            ...
            elseif discard_input == 'b'
                ...
                [first_saccade_time_spec, peak_vel_spec, Trial_3] = Saccade_Detector_GP_RTtask_spec1A(Trial, Trial_3, saccadeTimeFrame_0_spec,saccadeTimeFrame_1_spec, velocityThreshold_spec, accelerationThreshold_spec, w, allh, allv, DT, RT, FirstPassThresholds_spec);
                for n = w
                    Trial_3(n).first_saccade_time = first_saccade_time_spec;
                    Trial_3(n).RT =  first_saccade_time_spec -  Trial_3(n).GPOn;
                    Trial_3(n).PeakVelocity = peak_vel_spec;
                end
                analog_signal_plotter_GP(Trial,Trial_3,saccadeTimeFrame_0, w, DT, RT);    %graph the specific trial out
                prompt_2f_input = 'z';
            ...
            elseif discard_input == 'e'
                dbstop in 'SacDetectManualCheck_GP' at '130'
                tryagain = 0;
            ...
            elseif discard_input == 'g'
                Trial_3(n).RT = input(['Manually input the RT here: '])/1000;
                Trial_3(n).PeakVelocity = input(['Manually input the PV here: ']);
                prompt_2f_input = 'z';
                while prompt_2f_input ~= 'd' && prompt_2f_input ~= 'a' && prompt_2f_input ~= 't'
                    clc
                    prompt_2f = ['Keep or discard adjusted trial? "d" for keep, "a" for discard, "t" for tryagain: '];
